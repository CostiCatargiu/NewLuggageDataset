#!/usr/bin/env python3
"""
Full Dataset (No Aug) — 2 best ablation variants.

Current best on full: v5_cls12_tight_clip_full = 83.03%

Now testing the 2 ablation winners:
  1. v5_tal_alpha08_full2     — ablation test winner (74.20%)
  2. v5_alpha09_04_full       — ablation valid winner (73.56%), best generalization

Usage:
  python run_full_v5_variants.py
"""
...
EXPERIMENTS = [
    make_exp(
        "v5_tal_alpha08_full2",
        "Ablation test winner: tal_alpha 0.5->0.8",
        tal_alpha=0.8,
    ),
    make_exp(
        "v5_alpha09_04_full",
        "Ablation valid winner: alpha 0.9->0.4, best generalization",
        alpha_start=0.9,
        alpha_end=0.4,
        alpha_min=0.3,
        alpha_max=0.9,
...
def main():
    ...
    print(f"\n{'=' * 70}")
    print(f"  2 BEST ABLATION VARIANTS ON FULL (NO AUG)")
    print(f"  Current best: v5_cls12_tight_clip_full = 83.03%")
    print(f"{'=' * 70}")
    print(f"  Model:    {MODEL_WEIGHTS}")
    print(f"  Dataset:  {DATA_YAML}")
    print(f"  Epochs:   {EPOCHS}    Batch: {BATCH}")
    print(f"{'=' * 70}")
    print(f"  [1] v5_tal_alpha08_full2    tal_alpha=0.8")
    print(f"  [2] v5_alpha09_04_full      alpha 0.9->0.4")
    print(f"{'=' * 70}\n")
