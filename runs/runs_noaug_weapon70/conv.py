...
class LSKA_Residual(nn.Module):
    ...
    Problem: Standard LSKA multiplies features by attention (x * sigmoid(attn)).
    This can "wash out" small object features when the large kernel averages
    over a wide area.
    
    Solution: Add a learnable residual gate. Small objects can bypass the
    large kernel attention while large objects benefit from it.
    
    output = x * sigmoid(attn) * alpha + x * (1 - alpha)
    where alpha is a learned per-channel parameter (starts at 0.5).
    """
    ...
    def __init__(self, c1: int, k_size: int = 7):
        ...
        # Spatial: decomposed large kernel
        self.conv_h = nn.Conv2d(c1, c1, (1, k_size), padding=(0, pad), groups=c1, bias=False)
        self.conv_v = nn.Conv2d(c1, c1, (k_size, 1), padding=(pad, 0), groups=c1, bias=False)
        ...
        # Learnable residual gate (starts at 0.5 = equal mix)
        self.alpha = nn.Parameter(torch.ones(1, c1, 1, 1) * 0.5)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        attended = x * torch.sigmoid(attn)
        alpha = torch.sigmoid(self.alpha)
        return attended * alpha + x * (1 - alpha)


class LSKA_MultiScale(nn.Module):
    """
    Multi-scale LSKA for weapon detection.
    
    Problem: Single kernel size (7x7) is a compromise. Knives and pistols
    are compact (need smaller kernel), long guns are elongated (need larger).
    
    Solution: Two parallel LSKA branches with different kernel sizes (5 and 9),
    combined with learned channel-wise mixing weights.
    
    Small kernel (5): better for compact weapons (pistol, knife)
    Large kernel (9): better for elongated weapons (long gun)
    """

    def __init__(self, c1: int, k_small: int = 5, k_large: int = 9):
        super().__init__()
        
        pad_s = k_small // 2
        pad_l = k_large // 2
        
        # Shared channel mixing
        self.conv0 = nn.Conv2d(c1, c1, 1, bias=False)
        self.bn0 = nn.BatchNorm2d(c1)
        self.act = nn.SiLU(inplace=True)
        
        # Small kernel branch
        self.conv_h_s = nn.Conv2d(c1, c1, (1, k_small), padding=(0, pad_s), groups=c1, bias=False)
        self.conv_v_s = nn.Conv2d(c1, c1, (k_small, 1), padding=(pad_s, 0), groups=c1, bias=False)
        
        # Large kernel branch
        self.conv_h_l = nn.Conv2d(c1, c1, (1, k_large), padding=(0, pad_l), groups=c1, bias=False)
        self.conv_v_l = nn.Conv2d(c1, c1, (k_large, 1), padding=(pad_l, 0), groups=c1, bias=False)
        
        # Mixing: channel-wise gate to blend small and large
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Small kernel attention
        attn_s = self.conv_v_s(self.conv_h_s(attn))
        
        # Large kernel attention
        attn_l = self.conv_v_l(self.conv_h_l(attn))
        
        # Channel-wise mixing
        w = self.mix(x)  # per-channel weight based on input content
        attn = attn_s * (1 - w) + attn_l * w
        
        attn = self.bn1(self.conv1(attn))
        return x * torch.sigmoid(attn)


class LSKA_Weapon(nn.Module):
    """
    Weapon-optimized LSKA combining residual connection + multi-scale kernels.
    
    Combines the best of both improvements:
    1. Multi-scale: 5x5 for compact weapons + 9x9 for elongated weapons
    2. Residual gate: preserves small object features
    
    This is the full weapon-optimized variant.
    """
