...
# =============================================================================
# SATAL-SWA-Plus v3: Evidence-Driven Refinements (Round 8)
# =============================================================================
#
# Base: loss_satal_swa_plus_v2.py (all Rounds 1-7 features)
#
# v3 additions -- MULTIPLICATIVE REWEIGHTING ONLY (no new loss terms):
#   Section K: AR-aware weighting -- up-weights tall/thin GT objects to
#              compensate for the +77% AR shift between train and test.
#   Section L: Smooth SWA v2 -- sigmoid ramp replaces the hard step at
#              small_obj_px, combined with AR awareness in a joint weight.
#   Section M: WIoU class-focus -- per-class difficulty multiplier on the
#              WIoU attention factor (bag gets more box-loss focus).
#   Section N: IoU-conditioned DFL -- boosts DFL loss for moderate-IoU
#              boxes (IoU 0.5-0.8) that determine mAP50-95.
#
# Design principle: All features default to OFF. With default params,
# v3 behaves IDENTICALLY to v2. No new gradient directions -- only
# multiplicative reweighting of existing losses.
#
# =============================================================================
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max=16):
        ...
        # Section K: Aspect-ratio-aware weighting (Round 8 / v3)
        # Up-weights loss for GT boxes with AR > ar_threshold to compensate
        # for the train->test AR distribution mismatch (+77% median shift).
        self.use_ar_weight = False       # OFF by default (v2-compatible)
        self.ar_threshold = 2.0          # AR below this gets weight 1.0
        self.ar_max_weight = 1.5         # max multiplier at extreme AR
        self.ar_saturation = 4.0         # AR at which max_weight is reached

        # Section L: Smooth SWA v2 -- sigmoid ramp (Round 8 / v3)
        # Replaces the hard step function with a smooth sigmoid transition.
        # Only active when swa_smooth_v2=True AND small_obj_px > 0.
        self.swa_smooth_v2 = False       # OFF by default (v2-compatible)
        self.swa_sharpness = 5.0         # sigmoid steepness (higher = closer to hard step)

        # Section M: WIoU class-focus (Round 8 / v3)
        # Per-class multiplier on WIoU's box regression loss.
        # Derived from study results: bag is hardest (74% mAP50), trolley easiest (90%).
        self.use_class_focus = False      # OFF by default (v2-compatible)
        self.class_focus_weights = None   # set by set_params; [backpack, bag, trolley]

        # Section N: IoU-conditioned DFL sharpening (Round 8 / v3)
        # Boost DFL loss for moderate-IoU boxes (0.5-0.8) that determine mAP50-95.
        self.use_dfl_iou_weight = False   # OFF by default (v2-compatible)
        self.dfl_refine_boost = 0.5       # max extra weight on the Gaussian bump
        self.dfl_refine_center = 0.65     # IoU center of the bump
        self.dfl_refine_sigma = 0.15      # width of the bump
    ...
    def set_params(self, hyp):
        ...
        self.use_ar_weight = getattr(hyp, 'use_ar_weight', self.use_ar_weight)
        self.ar_threshold = getattr(hyp, 'ar_threshold', self.ar_threshold)
        self.ar_max_weight = getattr(hyp, 'ar_max_weight', self.ar_max_weight)
        self.ar_saturation = getattr(hyp, 'ar_saturation', self.ar_saturation)

        # Section L: Smooth SWA v2
        self.swa_smooth_v2 = getattr(hyp, 'swa_smooth_v2', self.swa_smooth_v2)
        self.swa_sharpness = getattr(hyp, 'swa_sharpness', self.swa_sharpness)

        # Section M: WIoU class-focus
        self.use_class_focus = getattr(hyp, 'use_class_focus', self.use_class_focus)
        cf_bp = getattr(hyp, 'class_focus_backpack', 1.1)
        cf_bag = getattr(hyp, 'class_focus_bag', 1.3)
        cf_tr = getattr(hyp, 'class_focus_trolley', 1.0)
        self.class_focus_weights = [cf_bp, cf_bag, cf_tr]

        # Section N: IoU-conditioned DFL
        self.use_dfl_iou_weight = getattr(hyp, 'use_dfl_iou_weight', self.use_dfl_iou_weight)
        self.dfl_refine_boost = getattr(hyp, 'dfl_refine_boost', self.dfl_refine_boost)
        self.dfl_refine_center = getattr(hyp, 'dfl_refine_center', self.dfl_refine_center)
        self.dfl_refine_sigma = getattr(hyp, 'dfl_refine_sigma', self.dfl_refine_sigma)
    ...
    def _compute_ar_weight(self, target_bboxes, fg_mask):
        """
        Section K: Aspect-ratio-aware weighting.
        Up-weights loss for GT boxes with extreme AR (tall/thin or wide/flat)
        to compensate for the train->test AR distribution mismatch.
        Returns (M, 1) weight tensor for foreground anchors.
        """
        if not self.use_ar_weight:
            return None

        fg_boxes = target_bboxes[fg_mask]  # (M, 4) xyxy
        w = (fg_boxes[..., 2] - fg_boxes[..., 0]).clamp(min=1e-6)
        h = (fg_boxes[..., 3] - fg_boxes[..., 1]).clamp(min=1e-6)
        ar = torch.max(h / w, w / h)  # always >= 1.0

        # Smooth linear ramp: 1.0 at ar_threshold, ar_max_weight at ar_saturation
        ramp = (ar - self.ar_threshold) / max(self.ar_saturation - self.ar_threshold, 1e-6)
        ramp = ramp.clamp(0.0, 1.0)
        ar_w = 1.0 + (self.ar_max_weight - 1.0) * ramp

        return ar_w.unsqueeze(-1)  # (M, 1)
    ...
    def _compute_weights(self, target_bboxes, target_scores, fg_mask, stride=None):
        ...
        if stride is not None and area_weight.numel() > 0:
            ...
            # Section L: Smooth SWA v2 -- sigmoid ramp (replaces hard step)
            if self.swa_smooth_v2 and self.small_obj_px > 0:
                # Sigmoid centered at small_threshold; objects at threshold/2 get
                # near-full boost, objects at threshold*2 get near-zero boost.
                x = (fg_areas - small_threshold) / small_threshold.clamp(min=1e-9)
                sigmoid_factor = torch.sigmoid(-self.swa_sharpness * x)
                boost = 1.0 + (self.small_obj_boost - 1.0) * sigmoid_factor
                area_weight = area_weight * boost.unsqueeze(-1)
            elif self.swa_smooth:
                # v2 continuous boost (kept for backward compatibility)
                ratio = (small_threshold / fg_areas.clamp(min=1e-9)).clamp(min=1.0)
                factor = ratio.pow(self.swa_boost_power).clamp(max=self.small_obj_boost)
        ...
        # Section K: AR-aware weighting (multiplicative on area_weight)
        ar_w = self._compute_ar_weight(target_bboxes, fg_mask)
        if ar_w is not None and area_weight.numel() > 0:
            area_weight = area_weight * ar_w
    ...
    def _regression_loss(self, pred_fg, target_fg, class_labels=None):
        ...
        if t == 'mpdiou':
            ...
            reg = self._wiou_loss(pred_fg, target_fg, class_labels=class_labels)
        elif t == 'focaler':
    ...
    def _wiou_loss(self, pred_fg, target_fg, eps=1e-7, class_labels=None):
        """
        Wise-IoU v3: non-monotonic dynamic focusing on ordinary-quality anchors.

        Section M (v3): optional per-class difficulty multiplier.
        When use_class_focus=True AND class_labels is provided, the WIoU
        attention factor is scaled by a class-specific weight so harder
        classes (bag) get more box-loss gradient.
        """
        ...
        # Section M: per-class focus multiplier on the WIoU loss
        if self.use_class_focus and class_labels is not None and self.class_focus_weights is not None:
            cfw = torch.tensor(self.class_focus_weights, device=loss.device, dtype=loss.dtype)
            focus = cfw[class_labels.clamp(0, len(self.class_focus_weights) - 1)]
            loss = loss * focus
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes,
        ...
        # Section M: extract class labels for WIoU class-focus
        class_labels = None
        if self.use_class_focus and target_scores.shape[-1] > 0:
            class_labels = target_scores[fg_mask].argmax(dim=-1)  # (M,)

        # =====================================================================
        # Compute box regression loss (IoU, NWD, or blend)
        # =====================================================================
        reg_loss = self._regression_loss(pred_fg, target_fg, class_labels=class_labels)
        ...
        # =====================================================================
        # Section N: IoU-conditioned DFL sharpening
        # Compute current IoU for DFL weighting (detached -- no gradient through this)
        # =====================================================================
        dfl_iou_weight = None
        if self.use_dfl_iou_weight and pred_fg.numel() > 0:
            with torch.no_grad():
                cur_iou = bbox_iou(pred_fg, target_fg, xywh=False, CIoU=False).squeeze(-1)
                # Gaussian bump centered at dfl_refine_center
                bump = torch.exp(-0.5 * ((cur_iou - self.dfl_refine_center) / self.dfl_refine_sigma) ** 2)
                dfl_iou_weight = (1.0 + self.dfl_refine_boost * bump).unsqueeze(-1)  # (M, 1)
        ...
        if self.dfl_loss:
            ...
            # Section N: apply IoU-conditioned DFL weight
            if dfl_iou_weight is not None:
                per_sample_dfl = per_sample_dfl * dfl_iou_weight
...
class v8DetectionLoss:
    ...
    def _print_config(self):
        ...
        if not hasattr(self, '_config_printed'):
            ...
            # v3 sections
            ar_on = getattr(self.bbox_loss, 'use_ar_weight', False)
            sv2_on = getattr(self.bbox_loss, 'swa_smooth_v2', False)
            cf_on = getattr(self.bbox_loss, 'use_class_focus', False)
            dfl_iou_on = getattr(self.bbox_loss, 'use_dfl_iou_weight', False)
            print(f"  [K] AR weighting:    {'ON' if ar_on else 'OFF'}" + (f" (thresh={self.bbox_loss.ar_threshold}, max={self.bbox_loss.ar_max_weight}, sat={self.bbox_loss.ar_saturation})" if ar_on else ""))
            print(f"  [L] Smooth SWA v2:   {'ON' if sv2_on else 'OFF'}" + (f" (sharpness={self.bbox_loss.swa_sharpness})" if sv2_on else ""))
            print(f"  [M] WIoU class-focus: {'ON' if cf_on else 'OFF'}" + (f" (bp={self.bbox_loss.class_focus_weights[0]}, bag={self.bbox_loss.class_focus_weights[1]}, tr={self.bbox_loss.class_focus_weights[2]})" if cf_on and self.bbox_loss.class_focus_weights else ""))
            print(f"  [N] DFL IoU-refine:  {'ON' if dfl_iou_on else 'OFF'}" + (f" (boost={self.bbox_loss.dfl_refine_boost}, center={self.bbox_loss.dfl_refine_center}, sigma={self.bbox_loss.dfl_refine_sigma})" if dfl_iou_on else ""))
