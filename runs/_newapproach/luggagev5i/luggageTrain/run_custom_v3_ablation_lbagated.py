#!/usr/bin/env python3
"""
loss_custom_v3 — ROUND 2: gated LBA (single pre-registered arm).
...
RUNS = [
    # Completed (results in test_full_dataset json):
    #   v3_anchor2    57.63  — reproduces v12s_default2 EXACTLY; chassis valid
    #   v3_lba2       57.57  — ungated: large +1.37 (trolley-first, matching
    #                 the footprint diagnostic s32 bias 0.18/0.22/0.63), but
    #                 s8 drained -12% fg, small -0.36, late val -0.27, P-1.7/R+0.4
    #   v3_ardfl_w 57.41 / v3_ardfl_h 57.02 / v3_dflgain17 56.91 / v3_peu 56.53
    #                 — DFL channel triple-eliminated; PEU killed with controls

    # ---- THIS ROUND: GATED LBA (single pre-registered arm) ----------------
    # Gate 96px on the same 'max' axis as the prior: LBA applies ONLY to GTs
    # that can plausibly live at s16-high/s32; small/medium keep stock
    # assignment BY CONSTRUCTION. Read THREE signatures before any verdict:
    #   1. telemetry.csv vs anchor's: s8 share within ~1pt ALL phases (gate
    #      didn't leak); s32 up ~+2-4pt (not ungated's +9pt). gate_pass_rate
    #      shows the mosaic interaction (expect a jump at ep 61).
    #   2. results.csv: late-phase (ep56-70) val delta vs anchor >= 0
    #      (ungated was -0.27) and the cls-loss/precision gap closed.
    #   3. diag_anchor_footprint.py on gated best.pt: s32 SEL BIAS 0.30 ->
    #      toward 1.0 with the s8 row UNCHANGED; CocoEval buckets: large
    #      keeps ~+1 to +1.4, small/medium ~0, trolley-large leading.
    # Decision, pre-committed: all pass -> new baseline (then optionally
    # gate 128 and/or strength 1.0 — cheap at 1.5h). Telemetry passes but
    # val still ~-0.2 -> precision cost is inherent to enriching P5 ->
    # LBA closes as a clean two-run kill. No other arms until this reads out.
    {"name": "v3_lba_gated", "kind": "assigner",
     "label": "LBA strength 0.8, ref_cells 6.0, axis 'max', GATE 96px",
     "params": dict(use_lba=True, lba_strength=0.8, lba_ref_cells=6.0,
                    lba_sigma=1.0, lba_size_axis="max", lba_size_gate_px=96.0)},
]
