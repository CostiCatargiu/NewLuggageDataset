...
RUNS = [
    ...
    #
    # # ---- THE LEAD ARM ----------------------------------------------------
    # # 2-epoch smoke: +4.05 mAP50-95 / +5.76 mAP50 / bag +6.2 vs anchor.
    # # MEASURED WEIGHTS (epoch 2): left 0.711  top 1.171  right 0.689  bottom 1.429
    # # i.e. norm_by_mu INVERTS the intended attenuation. mu_top/mu_left ~2.2 for a
    # # 41x90 box, so dividing by mu^2 (ratio 4.7) over-corrects the ~2.0 variance
    # # ratio and the height edges end up weighted UP, not down. What this actually
    # # runs is per-sample HEIGHT-weighted DFL at h/w ~1.85.
    # # warmup 0 to match exactly the config that produced the signal.
    # {"name": "v3_peu", "kind": "dfl",
    #  "label": "PEU beta=1.0 lambda=0 detach=True norm_by_mu=True warmup=0 (realises h/w ~1.85, learned)",
    #  "params": dict(use_peu=True, peu_beta=1.0, peu_lambda=0.0, peu_detach=True,
    #                 peu_norm_by_mu=True, peu_warmup_epochs=0,
    #                 peu_min_var=0.05, peu_w_clip=3.0)},
    #
    # # ---- CONTROL 1: is PEU's gain just h-heavy reweighting? ---------------
    # # Fixed weights matching PEU's realised ratio (1.171/0.711 = 1.65 top:left,
    # # 1.429/0.689 = 2.07 bottom:right -> ~1.85 mean). If this matches v3_peu,
    # # the adaptivity claim is dead and the finding is "weight the height edges".
    # # h=1.4 w=0.6 gives ratio 2.33 mean-normalised.
    # {"name": "v3_ardfl_h", "kind": "control",
    #  "label": "CONTROL fixed AR-DFL h=1.4 w=0.6 — does static h-heavy match learned PEU?",
    #  "params": dict(use_ardfl=True, ardfl_w_weight=0.6, ardfl_h_weight=1.4)},
    #
    # # ---- CONTROL 2: is PEU's gain just more DFL gain? ---------------------
    # # PEU's weights are mean-normalised (they sum to exactly 4.000) BUT they
    # # correlate with per-edge loss, so up-weighting the high-loss height edges
    # # raised dfl_loss 1.269 -> 1.437 (+13%). 1.5 * 1.132 = 1.70.
    # {"name": "v3_dflgain17", "kind": "control",
    #  "label": "CONTROL stock with dfl gain 1.7 (matches PEU's +13% dfl magnitude)",
    #  "params": dict(dfl=1.7)},
    #
    # # ---- DIRECTION TEST --------------------------------------------------
    # # The opposite sign, from the IoU-sensitivity derivation: 1px on a width edge
    # # buys 1/41 = 2.44% IoU, on a height edge 1/90 = 1.11%. Mean-normalised with
    # # w/h = 2.69 -> 1.458 / 0.542. Smoke test gave +0.89, i.e. also positive but
    # # far below the h-heavy arm — so the derivation is at best incomplete.
    # {"name": "v3_ardfl_w", "kind": "dfl",
    #  "label": "AR-DFL w=1.46 h=0.54 — width edges up (IoU-sensitivity derivation)",
    #  "params": dict(use_ardfl=True, ardfl_w_weight=1.458, ardfl_h_weight=0.542)},

    # ---- ASSIGNER --------------------------------------------------------
    # ref_cells=6.0 on the MAX axis -> nominal 48/96/192px at 640. Smoke test
    # moved P5 share 2.6% -> 5.9% (the prior works mechanically) but cost -1.67
    # at 2 epochs, trolley worst hit. Kept to confirm at convergence; expect it
    # to fail — P5 has median candidate pool 2 and 26.3% of GTs reach it with 0.
