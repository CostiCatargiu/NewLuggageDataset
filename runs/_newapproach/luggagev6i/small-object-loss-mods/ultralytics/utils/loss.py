...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        if self.nwd:  # NWD is absolute-scale, so score boxes in pixels rather than in per-level grid units
            s = stride.expand(fg_mask.shape[0], -1, -1)[fg_mask]
            pred_pos, target_pos = pred_bboxes[fg_mask] * s, target_bboxes[fg_mask] * s
        else:
        ...
        if self.dfl_loss:
            ...
            if self.obj_norm:  # image-size normalization scales this term with object size, the box itself does not
                wh = (target_bboxes[..., 2:] - target_bboxes[..., :2]).clamp(min=1e-9)
                norm = torch.cat((wh, wh), -1)  # (w, h, w, h) to match ltrb
                target_ltrb, pred_dist = target_ltrb / norm, pred_dist / norm
            else:
...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        self.assigner = TaskAlignedAssigner(
            ...
            nwd=h.nwd,
            nwd_c=h.nwd_c,
            scale_balance=h.scale_balance,
            iou_type=h.iou_type,
        )
        self.bbox_loss = BboxLoss(
            m.reg_max, nwd=h.nwd, nwd_c=h.nwd_c, iou_type=h.iou_type, obj_norm=h.dfl_obj_norm
        ).to(device)
