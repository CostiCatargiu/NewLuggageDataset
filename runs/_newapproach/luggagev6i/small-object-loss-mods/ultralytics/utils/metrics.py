...
# Maps the iou_type hyperparameter onto the bbox_iou flags, so loss and assignment stay on the same metric
IOU_FLAGS = {"iou": {}, "giou": {"GIoU": True}, "diou": {"DIoU": True}, "ciou": {"CIoU": True}, "eiou": {"EIoU": True}}
...
def bbox_iou(
    ...
    Args:
        ...
        EIoU (bool, optional): If True, calculate Efficient IoU. CIoU's aspect term can only trade width against
            height, never scale both together, so EIoU penalizes the two independently.
        NWD (float, optional): Blend ratio in [0.0, 1.0] mixing Normalized Wasserstein Distance into the result. IoU
            degrades sharply for a few pixels of error on tiny boxes, while NWD stays smooth and non-vanishing, so a
            non-zero ratio improves small object detection.
        nwd_c (float, optional): NWD normalization constant in the same units as the boxes, set to the mean object
            size of the dataset.
        eps (float, optional): A small value to avoid division by zero.

    Returns:
        (torch.Tensor): IoU, GIoU, DIoU, CIoU, or EIoU values depending on the specified flags, optionally blended
            with NWD.
    ...
    if NWD:  # 2nd Wasserstein distance between the boxes modeled as Gaussians, mapped to a similarity by exp(-d/c)
        # Halve before squaring and accumulate in fp32: these are pixel distances and overflow fp16 under autocast
        ...
        # Offset before the sqrt keeps its gradient finite as a prediction converges onto its target
