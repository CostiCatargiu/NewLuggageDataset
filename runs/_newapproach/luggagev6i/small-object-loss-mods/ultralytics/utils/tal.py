...
class TaskAlignedAssigner(nn.Module):
    ...
    Attributes:
        ...
        nwd (float): Blend ratio mixing Normalized Wasserstein Distance into the overlap metric.
        nwd_c (float): NWD normalization constant in pixels.
        scale_balance (float): Exponent equalizing each ground truth's contribution across object scales.
    ...
    def __init__(
        ...
        Args:
            ...
            nwd (float, optional): Blend ratio in [0.0, 1.0] mixing NWD into the overlap metric.
            nwd_c (float, optional): NWD normalization constant in pixels.
            scale_balance (float, optional): Exponent in [0.0, 1.0] equalizing each ground truth's contribution
                across object scales. 0.0 disables it, 1.0 fully removes the geometric bias.
            iou_type (str, optional): Overlap metric to assign with, one of the keys of IOU_FLAGS.
        """
        super().__init__()
        self.topk = topk
        ...
        self.iou_kwargs = IOU_FLAGS[iou_type]
        self.eps = eps

    def instance_gain(self, mask_pos):
        """Compute a per ground truth gain that equalizes its contribution across object scales.

        The anchors whose center falls inside a box grow with its area, so a large object wins several times more
        positives than a small one and dominates the loss purely through geometry. Rescaling each ground truth by
        how far its positive count sits from the batch mean removes that bias.

        Args:
            mask_pos (torch.Tensor): Positive mask with shape (b, max_num_obj, h*w).

        Returns:
            (torch.Tensor): Per ground truth gain with shape (b, max_num_obj, 1).
        """
        n_pos = mask_pos.sum(-1, keepdim=True)  # positives won by each gt, (b, max_num_obj, 1)
        valid = n_pos > 0
    ...
    def _forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        if self.scale_balance:
            norm_align_metric = norm_align_metric * self.instance_gain(mask_pos)
        target_scores = target_scores * norm_align_metric.amax(-2).unsqueeze(-1)
        if self.scale_balance:
            target_scores = target_scores.clamp_(max=1.0)  # boosting may overshoot the valid BCE target range
    ...
    def iou_calculation(self, gt_bboxes, pd_bboxes):
        ...
        """
        iou = bbox_iou(gt_bboxes, pd_bboxes, xywh=False, **self.iou_kwargs, NWD=self.nwd, nwd_c=self.nwd_c)
