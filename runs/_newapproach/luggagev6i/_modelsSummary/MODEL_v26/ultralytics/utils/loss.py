...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16):
        ...
        # Must match the assigner's setting: if the loss regresses one overlap definition
        # while the assignment selects by another, they optimise different objectives.
        # v8DetectionLoss sets both from the same two config keys for exactly that reason.
        self.nwd = 0.0
        self.nwd_c = 24.0  # IN PIXELS — see the grid-unit conversion in forward()
        self.iou_kwargs = IOU_FLAGS["ciou"]
    ...
    def forward(
        ...
        if sbb is not None:
            weight = weight * sbb
        pred_pos, target_pos = pred_bboxes[fg_mask], target_bboxes[fg_mask]
        if self.nwd:
            # Boxes arrive here in per-level GRID units (they were divided by stride).
            # CIoU is scale-invariant so that never mattered, but NWD is an ABSOLUTE-scale
            # metric and nwd_c is in pixels, so convert back before blending. Skipped
            # entirely when nwd == 0, which keeps the stock path bit-identical.
            #
            # stride is (A,) — one value per anchor across the flattened grid — while
            # fg_mask is (b, A). Broadcast it the same way sbb_weight() does, then add a
            # trailing axis so it multiplies the (M, 4) box coordinates.
            px = stride.view(1, -1).expand_as(fg_mask)[fg_mask].unsqueeze(-1)
            pred_pos, target_pos = pred_pos * px, target_pos * px
        iou = bbox_iou(pred_pos, target_pos, xywh=False, **self.iou_kwargs, NWD=self.nwd, nwd_c=self.nwd_c)
...
class v8DetectionLoss:
    ...
    def __init__(
        ...
        # ----------------------------------------------------- NWD / iou_type
        # Set on the ASSIGNER and the LOSS from the same keys. Upstream hardcoded
        # CIoU=True in both places independently; if they ever disagree the model
        # regresses one overlap definition while being assigned by another.
        iou_type = str(getattr(h, "iou_type", "ciou") or "ciou").lower()
        if iou_type not in IOU_FLAGS:
            raise ValueError(f"iou_type={iou_type!r} unknown; expected one of {sorted(IOU_FLAGS)}")
        nwd = float(getattr(h, "nwd", 0.0) or 0.0)
        nwd_c = float(getattr(h, "nwd_c", 24.0) or 24.0)
        if not 0.0 <= nwd <= 1.0:
            raise ValueError(f"nwd={nwd} outside [0, 1]")
        for obj in (self.assigner, bl):
            obj.nwd, obj.nwd_c, obj.iou_kwargs = nwd, nwd_c, IOU_FLAGS[iou_type]
        if nwd > 0.0 or iou_type != "ciou":
            LOGGER.info(f"overlap metric: iou_type={iou_type} nwd={nwd} nwd_c={nwd_c}px (assigner + loss)")

        # ----------------------------------------------------------------- SBAL
        # Scale-balanced assignment: per-GT gain equalising contribution across scales.
        sbal = float(getattr(h, "scale_balance", 0.0) or 0.0)
        if not 0.0 <= sbal <= 1.0:
            raise ValueError(f"scale_balance={sbal} outside [0, 1]")
        if sbal > 0.0:
            self.assigner.scale_balance = sbal
            LOGGER.info(f"SBAL active | scale_balance={sbal} (per-GT positives equalisation)")
