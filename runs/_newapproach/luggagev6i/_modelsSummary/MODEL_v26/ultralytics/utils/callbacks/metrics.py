...
# Overlap variant selector shared by BboxLoss and TaskAlignedAssigner. Both had CIoU=True
# hardcoded in separate places; routing them through one map means the loss and the
# assignment can no longer silently disagree about what "overlap" means.
IOU_FLAGS = {
    "iou": {},
    "giou": {"GIoU": True},
    "diou": {"DIoU": True},
    "ciou": {"CIoU": True},  # default — matches stock behaviour
    "eiou": {"EIoU": True},
}
...
def bbox_iou(
    ...
    # IoU
    iou = inter / union
    # NOTE: restructured from early-return to fall-through so the NWD block below can
    # blend with whichever variant was computed. With EIoU=False and NWD=0.0 the
    # arithmetic is identical to upstream — verified by the identity check in the runner.
    if CIoU or DIoU or GIoU or EIoU:
        ...
        if CIoU or DIoU or EIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            elif EIoU:  # https://arxiv.org/abs/2101.08158
                # CIoU's aspect term has dv/dw = -(h/w) * dv/dh, so its w and h gradients are
                # ALWAYS opposite in sign: it can trade width against height but never scale
                # both together. With 70.6% tall boxes at class-stable ratios that is the wrong
                # inductive bias. EIoU penalises the two extents independently.
                iou = iou - (rho2 / c2 + (w1 - w2).pow(2) / (cw.pow(2) + eps) + (h1 - h2).pow(2) / (ch.pow(2) + eps))
            else:
    ...
    if NWD:
        # 2nd Wasserstein distance between the boxes modelled as Gaussians, mapped to a
        # similarity by exp(-d/c). IoU is a RATIO, so a 3px error costs ~0.5 CIoU on a
        # 15x25px box and ~0.05 on a 100px one; raised to beta=6 in the assigner that is a
        # ~47x difference in assignment weight from the SAME absolute error. NWD is an
        # ABSOLUTE distance, so it degrades smoothly and identically at every scale.
        #
        # Three numerical guards, all load-bearing:
        #  - .float() and halve before squaring: these are PIXEL distances, and 640^2/4
        #    ~= 102,000 exceeds the fp16 max of 65504, so this overflows to inf under AMP.
        #  - (1e-3 * nwd_c)^2 offset before the sqrt: keeps its gradient finite as a
        #    prediction converges exactly onto its target.
        #  - exp(-d/c) is strictly positive, so align_metric never lands on exactly 0.
        #    That also defuses the topk tie-breaking-by-index-order issue for free.
        dx, dy = (b2_x1 + b2_x2 - b1_x1 - b1_x2).float() / 2, (b2_y1 + b2_y2 - b1_y1 - b1_y2).float() / 2
        dw, dh = (w1 - w2).float() / 2, (h1 - h2).float() / 2
        wd2 = dx.pow(2) + dy.pow(2) + dw.pow(2) + dh.pow(2)
        iou = (1 - NWD) * iou + NWD * (-(wd2 + (1e-3 * nwd_c) ** 2).sqrt() / nwd_c).exp()
    return iou
