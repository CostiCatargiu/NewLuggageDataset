...
from . import LOGGER
from .metrics import IOU_FLAGS, bbox_iou, probiou
...
class TaskAlignedAssigner(nn.Module):
    ...
    def __init__(
        ...
        # --- NWD / iou_type — inert at nwd = 0.0 and iou_type = "ciou" ----------
        # bbox_iou feeds BOTH the assignment metric (here) and BboxLoss. Everything else
        # in this project reweights that metric's OUTPUT; these two change what it
        # COMPUTES. See metrics.py::bbox_iou for the numerical guards.
        self.nwd = 0.0  # blend ratio in [0, 1]; 0 disables
        self.nwd_c = 24.0  # normalisation constant IN PIXELS (dataset mean object size)
        self.iou_kwargs = IOU_FLAGS["ciou"]  # stock

        # --- SBAL (Scale-Balanced assignment) — inert at scale_balance = 0.0 ----
        # select_candidates_in_gts accepts anchors whose CENTRE falls inside the box, so
        # the candidate pool grows with AREA while select_topk_candidates caps at topk.
        # A 12x18px object wins ~3 positives; a 39x55px one hits the cap of 10. That is a
        # 3-5x gradient difference from geometry alone, before any notion of difficulty,
        # and it affects the 35.8% of instances under 32px.
        # NOTE: target_scores is a BCE target, so boosting can push it above 1 where it is
        # clamped. The clamp binds for small objects that are ALREADY well fitted, so the
        # boost concentrates on small objects still fitting poorly. Defensible, but it is a
        # nonlinearity rather than a clean reweighting — disclose it in any write-up.
        self.scale_balance = 0.0
    ...
    def _forward(self, pd_scores, pd_bboxes, anc_points, gt_labels, gt_bboxes, mask_gt):
        ...
        # SBAL: equalise each GT's contribution across scales. Inert at scale_balance == 0.
        # Applied AFTER norm_align_metric so it scales the final per-GT target magnitude.
        if self.sbal_enabled():
            target_scores = target_scores * self.instance_gain(mask_pos)
    ...
    def sbal_enabled(self) -> bool:
        """True when scale-balanced assignment can change the loss (preflight no-op check)."""
        return float(self.scale_balance) != 0.0

    def instance_gain(self, mask_pos):
        """Per-GT gain equalising each ground truth's contribution across object scales.

        Anchors whose centre falls inside a box grow with its AREA, so a large object wins
        several times more positives than a small one and dominates the loss through
        geometry alone. Rescaling each GT by how far its positive count sits from the batch
        mean removes that bias. Normalising against the batch MEAN (not a constant) keeps
        the overall loss magnitude stable, so the box/cls balance and target_scores_sum
        do not drift.

        Args:
            mask_pos (torch.Tensor): Positive mask, shape (b, max_num_obj, h*w).

        Returns:
            (torch.Tensor): Per-GT gain, shape (b, max_num_obj, 1).
        """
        n_pos = mask_pos.sum(-1, keepdim=True)  # positives won by each gt
        valid = n_pos > 0
        mean_pos = n_pos.sum((1, 2), keepdim=True) / valid.sum((1, 2), keepdim=True).clamp(min=1)
        return (mean_pos / n_pos.clamp(min=1)).pow(self.scale_balance) * valid
    ...
    def iou_calculation(self, gt_bboxes, pd_bboxes):
        ...
        """
        # iou_kwargs selects the variant (default {"CIoU": True} = stock). NWD blends a
        # scale-invariant Gaussian-Wasserstein similarity in; both are inert at defaults.
        return (
            bbox_iou(gt_bboxes, pd_bboxes, xywh=False, **self.iou_kwargs, NWD=self.nwd, nwd_c=self.nwd_c)
            .squeeze(-1)
            .clamp_(0)
        )
