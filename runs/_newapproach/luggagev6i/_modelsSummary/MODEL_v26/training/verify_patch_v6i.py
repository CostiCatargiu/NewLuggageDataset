...
FILES = {
    ...
    "utils/tal.py": [
        ...
        ("def set_strides", 1),
        ("def instance_gain", 1),  # round 11: SBAL
        ("self.iou_kwargs", 1),  # round 11: NWD / iou_type
    ],
    # Round 11. metrics.py owns bbox_iou, which feeds BOTH the assigner and BboxLoss.
    # It was stock until now, so it was never in this list — but if it is missing while
    # tal.py/loss.py are patched, every call raises TypeError on the NWD kwarg. Worse,
    # if it is STALE, the restructured fall-through is absent and the CIoU path silently
    # keeps returning early: no crash, wrong arithmetic.
    "utils/metrics.py": [
        ("IOU_FLAGS", 1),
        ("EIoU", 1),
        ("NWD", 1),
...
def runtime_check():
    ...
    # Round 11: bbox_iou at defaults must be BITWISE identical to the old CIoU path.
    # metrics.py was restructured from early-return to fall-through so the NWD block
    # could see the computed variant. That edit looks obviously safe and silently is
    # not: it sits under both the assignment metric and the box loss, so a slip
    # corrupts every run without raising anything.
    try:
        import torch as _t

        from ultralytics.utils.metrics import IOU_FLAGS, bbox_iou

        a = _t.tensor([[10.0, 10.0, 50.0, 60.0]])
        b = _t.tensor([[12.0, 13.0, 48.0, 63.0]])
        inert = _t.equal(
            bbox_iou(a, b, xywh=False, CIoU=True),
            bbox_iou(a, b, xywh=False, **IOU_FLAGS["ciou"], NWD=0.0, nwd_c=24.0),
        )
        live = not _t.equal(
            bbox_iou(a, b, xywh=False, CIoU=True),
            bbox_iou(a, b, xywh=False, CIoU=True, NWD=0.5, nwd_c=46.0),
        )
        print(f"  [runtime] bbox_iou @ defaults == stock CIoU (bitwise)  : {inert}   <-- ROUND 11")
        print(f"  [runtime] bbox_iou @ nwd=0.5 actually differs          : {live}")
        ok = ok and inert and live
    except Exception as e:
        print(f"  [runtime] metrics.py NWD/EIoU check FAILED: {e}")
        ok = False
    if has_cls:
