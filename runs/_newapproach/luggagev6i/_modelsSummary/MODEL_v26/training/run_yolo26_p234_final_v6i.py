...
RUNS = [
    ...
    # ---- ADDED after the round landed. y26_p234rich_b3 came in at 79.99, the
    # campaign maximum on mAP50_small, with mAP50-95 EXACTLY at baseline (55.30)
    # and the best recall of any high-mAP config. It is n=1 and eight of nine
    # maxima in this project have regressed on repeat.
    #
    # EXPECTATION, WRITTEN BEFORE THE RUNS: it TIES y26_p234rich_b1. Every gap
    # between them is inside 2 sd of the b1 triple, and correcting 79.99 by the
    # +0.20 seed-0 bias measured on that triple gives ~79.79 against b1's 79.47.
    # The beta surface has been confirmed flat four separate times; b3 vs b1 on
    # one graph is exactly the comparison that finding predicts will be null.
    #
    # The reason to run it anyway is that 79.99 cannot go in a table at n=1, and
    # AR50_95_small — the axis this config was chosen for — has an sd of only
    # 0.195 in the b1 triple, the tightest metric in the group. If b3's +0.30 on
    # AR survives, "choose beta by its recall profile since it is flat on mAP"
    # becomes a reportable claim rather than a single draw.
    {"name": "y26_p234rich_b3_s1", "order": 6, "prior": 0.00, "seed": 1, "seed_of": "y26_p234rich_b3",
     "cfg": CFG_RICH, "depth": (4, 4), "beta": 3.0, "params": cfg(tal_beta=3.0),
     "ctrl": 79.99, "ctrl_ar": 68.01,
     "label": "seed 1 of the campaign maximum (79.99 small, m5095 at baseline)",
     "why": "Expected to REGRESS toward ~79.8 and tie b1's 79.47. Run because "
            "79.99 cannot be tabulated at n=1, not because a win is expected."},

    {"name": "y26_p234rich_b3_s2", "order": 7, "prior": 0.00, "seed": 2, "seed_of": "y26_p234rich_b3",
     "cfg": CFG_RICH, "depth": (4, 4), "beta": 3.0, "params": cfg(tal_beta=3.0),
     "ctrl": 79.99, "ctrl_ar": 68.01,
     "label": "seed 2 — n=3, matching b1 so the two can be compared like for like",
     "why": "b1 is at n=3 with sd 0.456. Only n=3 on b3 makes the comparison "
            "symmetric; at n=2 the intervals differ and the tie cannot be stated "
            "cleanly."},
