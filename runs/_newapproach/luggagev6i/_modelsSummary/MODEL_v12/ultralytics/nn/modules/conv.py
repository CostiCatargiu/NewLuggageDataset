...
class AttnPool2d(nn.Module):
    """Attention pooling — drop-in replacement for nn.AdaptiveAvgPool2d(1).

    WHY THIS EXISTS (LuggageDataset v6i). 7350 of 9138 train images are
    640x360. Letterboxed onto a 640x640 canvas that is 43.8% constant grey
    padding, so `nn.AdaptiveAvgPool2d(1)` puts nearly half of every global
    descriptor into padding. A fixed pad fraction would be an affine shift the
    following layer's bias could absorb, but mosaic tiles four images per
    sample, so the padded fraction VARIES per sample and the contamination is
    not a learnable constant.

    Pools with a learned spatial softmax instead (GCNet context modelling,
    Cao et al. 2019):

        a   = softmax_HW( conv1x1(x) )      # (B, 1, HW)
        out = sum_HW a * x                  # (B, C, 1, 1)

    Padding produces low activation, so it earns low weight automatically — no
    pad mask needed, which these modules cannot see anyway.

    ZERO-INITIALISED score conv => uniform softmax at init => output is EXACTLY
    the average pool it replaces. Any module swapping AdaptiveAvgPool2d(1) for
    this starts numerically identical and can only learn away from it, so
    pretrained transfer is unaffected.

    Usage — one-line swap inside any of the affected modules:
        self.pool = nn.AdaptiveAvgPool2d(1)   ->   self.pool = AttnPool2d(c1)

    Affected in this file (all still on avg-pool, swap at your discretion):
        ChannelAttention, ChannelSE, EnhancedChannelAttention, LuggageCBAM,
        LuggageCBAMv2, ScaleAdaptiveSpatialAttention, EMA, DeformableConv2d,
        AdaptiveShapeConv, ShapeSpatialAttention
    NOTE ScaleAdaptiveSpatialAttention / ShapeSpatialAttention pool a 2-channel
    (avg+max) spatial map, so they need AttnPool2d(2).
    """
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        b, c, h, w = x.shape
        a = self.score(x).view(b, 1, h * w).softmax(dim=-1)
        return torch.bmm(x.view(b, c, h * w), a.transpose(1, 2)).view(b, c, 1, 1)
...
class ChannelSE(nn.Module):
    ...
    def __init__(self, c1: int, reduction: int = 8) -> None:
        ...
        self.attn = nn.Sequential(
            AttnPool2d(c1),       # (B,C,H,W) -> (B,C,1,1) - global context
...
class EnhancedChannelAttention(nn.Module):
    ...
    def __init__(self, channels: int, reduction: int = 16) -> None:
        ...
        """
        super().__init__()
        self.avg_pool = AttnPool2d(channels)
