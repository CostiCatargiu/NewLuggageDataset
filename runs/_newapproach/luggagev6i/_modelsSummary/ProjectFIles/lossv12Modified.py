...
class LevelBalancedTaskAlignedAssigner(TaskAlignedAssigner):
    ...
    def __init__(self, topk=10, num_classes=80, alpha=0.5, beta=6.0,
        ...
        # `or` NOT a getattr default: a blank key in default.yaml parses to
        # None, so the attribute EXISTS and getattr's default never fires. The
        # None then reached `s_max, m_max = self.size_thresholds` and crashed
        # every size_cond run at epoch 0 with "cannot unpack non-iterable
        # NoneType object". size_budgets escaped it only because its use site
        # already read `self.size_budgets or self._DEFAULT_SIZE_BUDGETS`.
        self.size_thresholds = tuple(size_thresholds) if size_thresholds else (48.0, 96.0)
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):
        ...
        # `or` rather than a getattr default — see the note in the assigner's
        # __init__: default.yaml ships this key BLANK, which yaml parses to
        # None, so getattr finds the attribute and never applies its default.
        self.lbtal_size_thresholds = (
            getattr(h, 'lbtal_size_thresholds', None) or (48.0, 96.0))
