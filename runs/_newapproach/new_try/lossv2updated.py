...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):
        ...
        # M and N are alternatives, not stackable. M promotes low-IoU candidates
        # in a starved GT's ranking; N culls a supply-poor GT to its best few —
        # opposite pulls on the same thin small boxes. The if/elif below would
        # also silently drop SNATAL; fail loudly instead.
        if self.use_artal and self.use_snatal:
            raise ValueError(
                "use_artal and use_snatal are mutually exclusive alternatives "
                "(Sections M and N) — enable at most one per run."
            )
