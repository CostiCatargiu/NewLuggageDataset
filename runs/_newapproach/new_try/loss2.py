...
# =============================================================================
# Section N (v3): SUPPLY-NORMALIZED TAL ASSIGNER (SNA-TAL)
# =============================================================================
...
#
#   pool_g   = number of candidate anchors for GT g (metric > 0)
...
# the topk cap anyway). This is exactly the simulation in report table 6.
...
class SupplyNormalizedTaskAlignedAssigner(TaskAlignedAssigner):
    """TAL with a per-GT top-k budget proportional to candidate supply."""

    def __init__(self, topk=10, num_classes=80, alpha=0.5, beta=6.0,
                 rho=0.25, k_min=1, eps=1e-9):
        super().__init__(topk=topk, num_classes=num_classes, alpha=alpha, beta=beta, eps=eps)
        self.rho = rho
    ...
    def select_topk_candidates(self, metrics, largest=True, topk_mask=None):
        ...
        number of candidate anchors (metric > eps) for that GT.
        ...
        # Per-GT candidate pool size (how many anchors have a positive metric).
        pool = (metrics > self.eps).sum(-1)                       # (b, n)
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):
        ...
        # =====================================================================
        # Section N (v3): Supply-normalized TAL assigner (SNA-TAL) — see class
        ...
        self.snatal_kmin = getattr(h, 'snatal_kmin', 1)
        ...
        if self.use_satal:
            ...
            # Section N (v3): supply-normalized TAL — per-GT k_eff by pool size
    ...
    def _print_config(self):
        ...
        if not hasattr(self, '_config_printed'):
            ...
            print(f"  [N] snatal:          {self.use_snatal}" + (f" (rho={self.snatal_rho}, k_min={self.snatal_kmin})" if self.use_snatal else ""))
            if self.use_satal:
