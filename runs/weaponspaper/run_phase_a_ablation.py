...
ALPHA_START_VALUES = [0.7, 0.6, 0.7, 0.8, 0.9, 1.0]
ALPHA_END_VALUES = [0.4, 0.4, 0.5, 0.6, 0.7, 0.8]
...
COCO_ANN_FILE = "/home/constantin/Doctorat/LuggageDataset_v2i_YOLOV12_30percentagesubset/annotations_coco_valid.json"
...
def _stats_to_dict(stats) -> Dict:
    """Convert a COCOeval.stats array to a labeled dict (AP + AR).

    COCOeval.stats layout (12 entries):
        [0]  AP @ IoU=.50:.95 | area=all    | maxDets=100
        [1]  AP @ IoU=.50     | area=all    | maxDets=100
        [2]  AP @ IoU=.75     | area=all    | maxDets=100
        [3]  AP @ IoU=.50:.95 | area=small  | maxDets=100
        [4]  AP @ IoU=.50:.95 | area=medium | maxDets=100
        [5]  AP @ IoU=.50:.95 | area=large  | maxDets=100
        [6]  AR @ IoU=.50:.95 | area=all    | maxDets=1
        [7]  AR @ IoU=.50:.95 | area=all    | maxDets=10
        [8]  AR @ IoU=.50:.95 | area=all    | maxDets=100
        [9]  AR @ IoU=.50:.95 | area=small  | maxDets=100
        [10] AR @ IoU=.50:.95 | area=medium | maxDets=100
        [11] AR @ IoU=.50:.95 | area=large  | maxDets=100
    """
    return {
        # Average Precision
...
def _empty_metrics() -> Dict:
    """Return a zero-filled metrics dict matching the schema of run_coco_evaluation."""
    zero_stats = {
        "mAP50_95":   0.0,
        "mAP50":      0.0,
        "mAP75":      0.0,
    ...
    return {**zero_stats, "per_class": {}}


def run_coco_evaluation(pred_json: Path, ann_file: str) -> Dict:
    """Run COCO evaluation with size-based AND per-class AP/AR metrics.

    Returns a dict containing overall AP/AR (size-stratified) plus a
    `per_class` sub-dict with the same metric schema for each category.
    """
    try:
        ...
        if not Path(ann_file).exists() or not pred_json.exists():
            print(f"    ERROR: missing file. "
                  f"ann_exists={Path(ann_file).exists()}, "
                  f"pred_exists={pred_json.exists()}")
            return _empty_metrics()

        coco_gt = COCO(str(ann_file))

        # Build filename -> id map for prediction conversion
        filename_to_id = {}
        ...
        if not converted:
            print("    WARNING: no predictions matched annotations after conversion")
            return _empty_metrics()
        ...
        coco_eval.params.areaRngLbl = area_lbl
        coco_eval.evaluate()
        coco_eval.accumulate()
        coco_eval.summarize()

        metrics = _stats_to_dict(coco_eval.stats)
        ...
        for cid in cat_ids:
            ...
            ce.params.areaRng = area_rng
            ce.params.areaRngLbl = area_lbl
            ce.params.catIds = [cid]
            ce.evaluate()
            ce.accumulate()
            ce.summarize()

            per_class[cat_name] = _stats_to_dict(ce.stats)
