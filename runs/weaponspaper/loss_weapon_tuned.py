...
# Weapon-Dataset-Tuned Loss — based on loss_custom_git.py
#
# Changes from loss_custom_git.py, tuned for the weapon detection dataset:
#
#   Dataset characteristics driving these changes:
#     - 84% large objects, only 1.4% small (at COCO 32/96 threshold)
#     - 1.3 instances/image (very sparse — massive negative anchor imbalance)
#     - 4 classes: pistol 37%, long_gun 30%, knife 17%, other 16%
#     - Elongated objects (knives, long guns) with median H/W ratio 1.23
#
#   [CHANGE 1] Classification: BCE -> VarifocalLoss
#       Why: With only 1.3 obj/img, negatives vastly outnumber positives.
#       VFL down-weights easy negatives, focuses on hard positives.
#       Controlled by: use_vfl (default True), vfl_alpha, vfl_gamma
#
#   [CHANGE 2] Small object params: px=40, boost=2.5
#       Why: Old px=70 was too loose (covered 12.5% of objects including mediums).
#       Real small objects are <32px (1.4%). Tighter threshold + stronger boost.
#
#   [CHANGE 3] TAL: topk=13, beta=4.0
#       Why: beta=6.0 made IoU dominate anchor assignment. For small weapons,
#       IoU is noisy (1px shift = large IoU change). Lower beta reduces noise.
#       More topk candidates help in sparse scenes.
#
#   [CHANGE 4] Alpha schedule: 0.7->0.3 (was 0.9->0.5)
#       Why: With 84% large objects, aggressive area-weighting early (0.9) is
#       too strong. 0.7 is enough. End at 0.3 to let score-weight dominate
#       in late training when the model has learned size patterns.
#
#   [CHANGE 5] IoU metric: CIoU -> DIoU (configurable)
#       Why: CIoU's aspect ratio penalty hurts elongated weapons (knives).
#       DIoU focuses on center-distance + overlap only.
#       Controlled by: iou_type ('CIoU', 'DIoU', 'GIoU', 'SIoU')
#
#   [CHANGE 6] Clipping: loosened (50->20 iou, 25->10 dfl)
#       Why: Old 20->10 clips were too aggressive. Hard small objects have
#       high loss values; clipping them suppresses their gradients.
#
#   [CHANGE 7] Config print: shows cls gain recommendation
#       Why: Default cls=0.5 is low for 4-class sparse scenes. Suggest cls=1.0.
...
class VarifocalLoss(nn.Module):
    ...
    Key for sparse-scene datasets: down-weights easy negatives, focuses on
    hard positives. Much better than BCE when objects/image ratio is low.
    """
...
class BboxLoss(nn.Module):
    """
    Bounding box loss — weapon-dataset-tuned defaults.

    Changes from loss_custom_git.py:
        [CHANGE 2] small_obj_px: 70 -> 40, small_obj_boost: 1.5 -> 2.5
        [CHANGE 4] alpha_start: 0.9 -> 0.7, alpha_end: 0.5 -> 0.3
        [CHANGE 5] iou_type: CIoU -> DIoU (configurable)
        [CHANGE 6] clipping: loosened
    """
    ...
    def __init__(self, reg_max=16):
        ...
        # [CHANGE 2] Section A: Tighter small-object threshold + stronger boost
        self.small_obj_px = 40          # was 70
        self.small_obj_boost = 2.5      # was 1.5

        # [CHANGE 4] Section A: Less aggressive alpha schedule
        self.alpha_start = 0.7          # was 0.9
        self.alpha_end = 0.3            # was 0.5
        self.alpha_min = 0.2            # was 0.3
        self.alpha_max = 0.8            # was 0.9

        # [CHANGE 5] IoU type: DIoU instead of CIoU
        self.iou_type = 'DIoU'          # was implicitly CIoU

        # [CHANGE 6] Section C: Loosened clipping
        self.iou_clip_start = 50.0      # was 20.0
        self.iou_clip_end = 20.0        # was 10.0
        self.dfl_clip_start = 25.0      # was 10.0
        self.dfl_clip_end = 10.0        # was 5.0
    ...
    def _get_dynamic_alpha(self):
        ...
        if self.epoch != self._last_logged_epoch:
            if self.epoch % 1 == 0:
                print(f"[Alpha] Epoch {self.epoch}/{self.total_epochs}: "
                      f"\u03b1={alpha:.3f} | IoU={self.iou_type}")
    ...
    def _compute_weights(self, target_bboxes, target_scores, fg_mask, stride=None):
        ...
        # [CHANGE 2] Apply small object boost with tighter threshold
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes,
        ...
        # [CHANGE 5] IoU loss — configurable metric
        iou_type = self.iou_type.upper()
        iou = bbox_iou(
            pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False,
            CIoU=(iou_type == 'CIOU'),
            DIoU=(iou_type == 'DIOU'),
            GIoU=(iou_type == 'GIOU'),
            # SIoU not in default ultralytics bbox_iou, falls back to IoU
        )
        per_sample_iou_loss = (1.0 - iou) * weight

        # [CHANGE 6] Get loosened adaptive clip values
...
class v8DetectionLoss:
    """
    YOLOv8 Detection Loss — weapon-dataset-tuned.

    All custom parameters are read from model.args (hyperparameters).
    Pass them via model.train(..., param_name=value, ...).

    Weapon-specific changes from loss_custom_git.py:
        [1] Classification: VFL instead of BCE (use_vfl=True)
        [2] Small object: px=40, boost=2.5 (was 70, 1.5)
        [3] TAL: topk=13, beta=4.0 (was 10, 6.0)
        [4] Alpha: 0.7->0.3 (was 0.9->0.5)
        [5] IoU: DIoU instead of CIoU (iou_type='DIoU')
        [6] Clipping: loosened
        [7] Config print: shows cls gain recommendation
    """

    def __init__(self, model, tal_topk=10):
        """Initialize v8DetectionLoss with weapon-tuned parameters."""
        ...
        # =====================================================================
        # READ PARAMETERS FROM model.args (weapon-tuned defaults)
        # =====================================================================
        ...
        # [CHANGE 2] Section A: Tighter small-object targeting
        self.small_obj_px = getattr(h, 'small_obj_px', 40)         # was 70
        self.small_obj_boost = getattr(h, 'small_obj_boost', 2.5)  # was 1.5

        # [CHANGE 4] Section A: Less aggressive alpha schedule
        self.alpha_start = getattr(h, 'alpha_start', 0.7)          # was 0.9
        self.alpha_end = getattr(h, 'alpha_end', 0.3)              # was 0.5
        self.alpha_min = getattr(h, 'alpha_min', 0.2)              # was 0.3
        self.alpha_max = getattr(h, 'alpha_max', 0.8)              # was 0.9

        # Section B: Center loss (unchanged)
        ...
        # [CHANGE 6] Section C: Loosened clipping
        self.iou_clip_start = getattr(h, 'iou_clip_start', 50.0)   # was 20.0
        self.iou_clip_end = getattr(h, 'iou_clip_end', 20.0)       # was 10.0
        self.dfl_clip_start = getattr(h, 'dfl_clip_start', 25.0)   # was 10.0
        self.dfl_clip_end = getattr(h, 'dfl_clip_end', 10.0)       # was 5.0

        # [CHANGE 3] Section D: TAL parameters — less IoU-dominant
        self.tal_topk = getattr(h, 'tal_topk', 13)                 # was 10
        self.tal_alpha = getattr(h, 'tal_alpha', 0.5)              # unchanged
        self.tal_beta = getattr(h, 'tal_beta', 4.0)                # was 6.0

        # [CHANGE 5] IoU type
        self.iou_type = getattr(h, 'iou_type', 'DIoU')             # was CIoU

        # [CHANGE 1] Classification loss: VFL vs BCE
        self.use_vfl = getattr(h, 'use_vfl', True)                 # NEW
        self.vfl_alpha = getattr(h, 'vfl_alpha', 0.75)             # VFL alpha
        self.vfl_gamma = getattr(h, 'vfl_gamma', 2.0)              # VFL gamma
        ...
        self.vfl = VarifocalLoss()  # [CHANGE 1]
        self.bbox_loss = BboxLoss(m.reg_max).to(device)

        # Pass parameters to BboxLoss
        self.bbox_loss.set_params(h)
        # Also propagate iou_type to BboxLoss
        self.bbox_loss.iou_type = self.iou_type

        # [CHANGE 3] Task Aligned Assigner with weapon-tuned parameters
    ...
    def _print_config(self):
        ...
        if not hasattr(self, '_config_printed'):
            print("\n" + "=" * 60)
            print("v8DetectionLoss Configuration (WEAPON-TUNED)")
            print("=" * 60)
            print(f"  [A] alpha_start:     {self.alpha_start}  (custom: 0.7)")
            print(f"  [A] alpha_end:       {self.alpha_end}  (custom: 0.3)")
            print(f"  [A] small_obj_px:    {self.small_obj_px}  (custom: 40)")
            print(f"  [A] small_obj_boost: {self.small_obj_boost}  (custom: 2.5)")
            print(f"  [B] center_loss_init:     {self.center_loss_weight_init}")
            print(f"  [B] center_loss_min:      {self.center_loss_weight_min}")
            print(f"  [C] iou_clip:        {self.iou_clip_start} -> {self.iou_clip_end}  (custom: loosened)")
            print(f"  [C] dfl_clip:        {self.dfl_clip_start} -> {self.dfl_clip_end}  (custom: loosened)")
            print(f"  [D] tal_topk:        {self.tal_topk}  (custom: 13)")
            print(f"  [D] tal_alpha:       {self.tal_alpha}")
            print(f"  [D] tal_beta:        {self.tal_beta}  (custom: 4.0)")
            print(f"  [NEW] iou_type:      {self.iou_type}  (custom: DIoU)")
            print(f"  [NEW] use_vfl:       {self.use_vfl}  (VarifocalLoss)")
            if self.use_vfl:
                print(f"  [NEW] vfl_alpha:     {self.vfl_alpha}")
                print(f"  [NEW] vfl_gamma:     {self.vfl_gamma}")
            print(f"  epochs:              {self.total_epochs}")
            print("-" * 60)
            cls_gain = getattr(self.hyp, 'cls', 0.5)
            if cls_gain <= 0.5:
                print(f"  TIP: cls gain is {cls_gain}. For 4-class weapon dataset")
                print(f"       with sparse scenes, consider cls=1.0 in your")
                print(f"       model.train() call for better discrimination.")
            print("=" * 60 + "\n")
    ...
    def __call__(self, preds, batch):
        ...
        # ── [CHANGE 1] Classification loss: VFL or BCE ──
        if self.use_vfl:
            # VarifocalLoss: down-weights easy negatives, key for sparse scenes
            target_labels = (target_scores > 0).float()
            loss[1] = self.vfl(
                pred_scores, target_scores.to(dtype), target_labels,
        ...
        if fg_mask.sum():
            ...
            # IoU and DFL losses (with CHANGE 2, 4, 5, 6 inside BboxLoss)
