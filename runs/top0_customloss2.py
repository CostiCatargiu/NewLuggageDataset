...
# ═══════════════════════════════════════════════════════════════════════════════
# LUGGAGE-SPECIALIZED LOSS COMPONENTS
# ═══════════════════════════════════════════════════════════════════════════════
...
class SizeAwareBboxLoss(nn.Module):
    ...
    Small objects (< ~32x32 px) receive higher loss weight to compensate
    for their underrepresentation in gradient magnitude. Small boxes produce
    small absolute IoU losses even when predictions are poor.

    Dataset context:
        - Small objects: 26.4% of instances (< 32x32 at 640x640)
        - Medium objects: 57.8% of instances
        - Large objects: 15.8% of instances (> 96x96 at 640x640)
        - Trolley class has highest small-object rate (28.4%)
    """
    ...
    def __init__(self, reg_max=16, small_threshold=0.0025,
        ...
        Args:
            ...
            small_threshold (float): Relative area threshold below which objects are 'small'.
                                     0.0025 corresponds to ~32x32 at 640x640.
            large_threshold (float): Relative area threshold above which objects are 'large'.
                                     0.0225 corresponds to ~96x96 at 640x640.
        ...
        self.size_weights = size_weights or {'small': 2.0, 'medium': 1.0, 'large': 0.8}

    def _compute_size_weight(self, target_bboxes_fg, stride_tensor_fg):
        """
        Compute per-object size weighting based on absolute pixel area.

        Args:
            target_bboxes_fg (torch.Tensor): (num_fg, 4) in stride-normalized xyxy coords.
            stride_tensor_fg (torch.Tensor): (num_fg, 1) stride for each foreground anchor.

        Returns:
            size_weight (torch.Tensor): (num_fg, 1) multiplicative weight per object.
        """
        # Convert to absolute pixel coordinates
        abs_bboxes = target_bboxes_fg * stride_tensor_fg  # (num_fg, 4) in pixels

        # Compute width and height in pixels
        widths = abs_bboxes[:, 2] - abs_bboxes[:, 0]
        heights = abs_bboxes[:, 3] - abs_bboxes[:, 1]

        # Normalize to fractional area (consistent with dataset analysis script)
        img_area = 640.0 * 640.0
        relative_area = (widths * heights) / img_area

        # Assign size category weights
        size_weight = torch.ones_like(relative_area)
        small_mask = relative_area < self.small_threshold
        large_mask = relative_area > self.large_threshold
        medium_mask = ~small_mask & ~large_mask

        size_weight[small_mask] = self.size_weights['small']
        size_weight[medium_mask] = self.size_weights['medium']
        size_weight[large_mask] = self.size_weights['large']
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes,
        ...
        Same interface as BboxLoss.forward() with an additional stride_tensor
        argument for computing absolute object sizes.
        ...
        Returns:
            loss_iou (torch.Tensor): IoU loss value.
            loss_dfl (torch.Tensor): DFL loss value.
        """
        ...
        # Size-aware weight
        size_weight = self._compute_size_weight(target_bboxes[fg_mask], stride_fg)
...
# ═══════════════════════════════════════════════════════════════════════════════
# LUGGAGE-SPECIALIZED DETECTION LOSS
# ═══════════════════════════════════════════════════════════════════════════════


class v8DetectionLossLuggage:
    """
    Specialized detection loss for luggage datasets (backpack / bag / trolley).

    Improvements over vanilla v8DetectionLoss:
        1. Class-weighted BCE/VFL to handle trolley dominance
           (51.3% trolley vs 21.9% bag).
        2. Size-aware IoU loss weighting — upweights small objects that are
           otherwise drowned out by the medium/large majority.
        3. Varifocal Loss for better soft-label learning in dense scenes
           (~4.5 objects per image).
        4. Configurable thresholds and weights for easy tuning.

    Dataset statistics this was designed for:
        Classes:   backpack=26.7%, bag=21.9%, trolley=51.3%
        Sizes:     small(<32x32)=26.4%, medium=57.8%, large(>96x96)=15.8%
        Density:   ~4.5 instances per image
        Images:    29,053 total (87/10/3 train/val/test split)
    """
    ...
    def __init__(self, model, tal_topk=10):
        ...
        # Approx result: backpack≈1.08, bag≈1.19, trolley≈0.78

        # BCE with no reduction — class weights applied manually
        self.bce = nn.BCEWithLogitsLoss(reduction="none")

        # ──────────────────────────────────────────────────────────
        # 2. SIZE-AWARE BBOX LOSS
        # ──────────────────────────────────────────────────────────
        # Thresholds as fraction of image area (from dataset analysis)
        self.small_threshold = 0.0025   # ~32x32 at 640x640
        self.large_threshold = 0.0225   # ~96x96 at 640x640
        self.size_weights = {
            'small': 2.0,   # 26.4% of data, hardest to detect
            'medium': 1.0,  # 57.8% of data, baseline
            'large': 0.8,   # 15.8% of data, easiest
        }
        ...
        # ──────────────────────────────────────────────────────────
        # 5. VARIFOCAL LOSS TOGGLE
        # ──────────────────────────────────────────────────────────
        # VFL is better for dense scenes with soft label assignment
        self.use_varifocal = True

        print(f"[LuggageLoss] Initialized with:")
        print(f"  Class weights (backpack/bag/trolley): {self.class_weights.cpu().numpy().round(4)}")
        print(f"  Size weights: {self.size_weights}")
        print(f"  Size thresholds: small<{self.small_threshold}, large>{self.large_threshold}")
        print(f"  Varifocal loss: {self.use_varifocal}")
    ...
    def _compute_cls_loss_weighted(self, pred_scores, target_scores, fg_mask,
        ...
        Args:
            pred_scores (torch.Tensor): (bs, num_anchors, nc) predicted class logits.
            target_scores (torch.Tensor): (bs, num_anchors, nc) target soft scores.
            fg_mask (torch.Tensor): (bs, num_anchors) boolean foreground mask.
            target_labels_for_fg (torch.Tensor): (num_fg,) integer class indices.
            target_scores_sum (float): Normalization factor.

        Returns:
            loss (torch.Tensor): Scalar classification loss.
        """
        dtype = pred_scores.dtype

        if self.use_varifocal:
            loss = self._varifocal_loss_weighted(
                pred_scores, target_scores, fg_mask, target_labels_for_fg
            )
        else:
    ...
    def _varifocal_loss_weighted(self, pred_score, gt_score, fg_mask,
                                  target_labels_for_fg, alpha=0.75, gamma=2.0):
        """
        Class-weighted Varifocal Loss.

        VFL focuses more on high-quality positive examples and less on easy
        negatives, which is ideal for dense luggage scenes (~4.5 obj/img).

        Args:
            pred_score (torch.Tensor): (bs, num_anchors, nc) predicted logits.
            gt_score (torch.Tensor): (bs, num_anchors, nc) target soft scores.
            fg_mask (torch.Tensor): (bs, num_anchors) boolean foreground mask.
            target_labels_for_fg (torch.Tensor): (num_fg,) integer class indices.
            alpha (float): Background loss weighting factor.
            gamma (float): Focal modulation exponent.

        Returns:
            loss (torch.Tensor): Scalar VFL loss (unnormalized).
        """
        # Positive/negative labeling
        label = (gt_score.sum(-1, keepdim=True) > 0).float().expand_as(gt_score)
