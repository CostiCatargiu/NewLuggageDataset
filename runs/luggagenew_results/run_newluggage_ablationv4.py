...
Runs (each config PINS every toggle for reproducibility — no default drift):
  1. r4_baseline_clean — SWA const06, CIoU, class-weighting OFF (true baseline)
  2. r4_mpdiou         — SWA const06 + MPDIoU box loss
  3. r4_wiou           — SWA const06 + Wise-IoU v3 box loss
  4. r4_focaler        — SWA const06 + Focaler-CIoU box loss
  5. r4_nwd_small      — SWA const06 + NWD small_only (C=4.0) on CIoU base
  6. r4_swa_smooth     — SWA const06 with continuous boost (stride-fixed)
  7. r4_satal_mpdiou   — SATAL R3 assignment + MPDIoU  ("have both": recall + tight)
  8. r4_satal_wiou     — SATAL R3 assignment + Wise-IoU v3
...
PREFLIGHT — MANDATORY before launch:
  ...
  [ ] ultralytics/utils/satal.py importable (runs 7-8)
  [ ] per-anchor stride fix now lives in BboxLoss._compute_weights (this file)

Sanity checks on epoch 1 banner:
  - run 1: box_loss_type ciou, Class Weighting OFF, use_satal False
  - runs 2-4: box_loss_type mpdiou/wiou/focaler, [Alpha] flat 0.600
  - run 5: use_nwd True, nwd_mode small_only
  - runs 7-8: use_satal True, box_loss_type mpdiou/wiou
...
_PINS = dict(
    ...
    use_class_weighting=True,      # keep ON to match Rounds 1-3 (except run 1)
    cls_mode="bce",
)


def _cfg(**overrides):
    """Build a run config: pins -> SWA-const06 base -> per-run overrides."""
    c = dict(_PINS)
...
RUNS = [
    {"name": "r4_baseline_clean", "label": "SWA const06, CIoU, class-weighting OFF -- clean baseline",              "params": R4_BASELINE_CLEAN, "seed": 0},
    {"name": "r4_mpdiou",         "label": "SWA const06 + MPDIoU box loss (tight localization)",                    "params": R4_MPDIOU,         "seed": 0},
    {"name": "r4_wiou",           "label": "SWA const06 + Wise-IoU v3 (1.9/3.0, mom 0.02)",                         "params": R4_WIOU,           "seed": 0},
    {"name": "r4_focaler",        "label": "SWA const06 + Focaler-CIoU (d0.0/u0.95)",                               "params": R4_FOCALER,        "seed": 0},
    {"name": "r4_nwd_small",      "label": "SWA const06 + NWD small_only (C=4.0) on CIoU base",                     "params": R4_NWD_SMALL,      "seed": 0},
    {"name": "r4_swa_smooth",     "label": "SWA const06 continuous boost (stride-fixed, power 0.5)",                "params": R4_SWA_SMOOTH,     "seed": 0},
    {"name": "r4_satal_mpdiou",   "label": "SATAL R3 assignment + MPDIoU -- recall AND tight boxes",                "params": R4_SATAL_MPDIOU,   "seed": 0},
    {"name": "r4_satal_wiou",     "label": "SATAL R3 assignment + Wise-IoU v3",                                     "params": R4_SATAL_WIOU,     "seed": 0},
]
