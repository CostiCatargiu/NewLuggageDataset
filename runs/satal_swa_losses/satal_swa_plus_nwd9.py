...
# =============================================================================
# SATAL-SWA-Plus-NWD9: Higher NWD Ratio for Small Objects
# =============================================================================
#
# Base: NWD4 (the best performer)
#
# NWD9 Change:
#   - nwd_ratio: 0.1 → 0.2 (doubled)
#   - Everything else UNCHANGED from NWD4
#
# Rationale:
#   - NWD4 proved that NWD helps small objects
#   - NWD8 failed because we changed C (broke cross-scale transfer)
#   - NWD9 keeps C=3.0 fixed, only increases NWD influence
#   - ratio=0.2 means: 80% CIoU + 20% NWD (still conservative)
#
# Expected:
#   - Small objects: better (more NWD help)
#   - Large objects: same or better (cross-scale transfer preserved)
#
# All NWD4 features preserved:
#   1. Symmetric normalization (geometric mean of pred/target dims)
#   2. Gaussian kernel exp(-w2/C²) — no gradient singularity
#   3. Soft sigmoid gate — smooth gradient transition
#   4. C = 3.0 (unchanged)
#   5. Auto-disable small_obj_boost when NWD is on
#   6. Minimum dimension clamp (0.5 grid units)
#
# =============================================================================
...
# =============================================================================
# BBOX LOSS WITH NWD9 (Higher Ratio)
# =============================================================================


class BboxLoss(nn.Module):
    """
    Bounding box loss with SWA + NWD9 (Higher NWD Ratio).

    NWD9 = NWD4 with nwd_ratio increased from 0.1 to 0.2
    ...
    def __init__(self, reg_max=16):
        ...
        # NWD9 defaults (same as NWD4, except ratio doubled)
        self.use_nwd = True
        self.nwd_ratio = 0.2               # [NWD9] doubled from 0.1 → 0.2
        self.nwd_constant = 3.0             # unchanged from NWD4
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes,
        ...
        if self.use_nwd:
            ...
            if self._nwd_dbg_count < 30:
                ...
                print(f"[NWD4] nwd mean={nwd.mean().item():.4f} "
...
class v8DetectionLoss:
    """
    SATAL-SWA-Plus-NWD9 Detection Loss.

    Base: NWD4 (the best performer)
    NWD9: Higher NWD ratio (0.2 instead of 0.1)
    ...
    All NWD4 features preserved:
      ...
      - C = 3.0 (unchanged)
      - Auto-disabled small_obj_boost (no redundant emphasis)
    """
    ...
    def __init__(self, model, tal_topk=10):
        ...
        # =====================================================================
        # Section H: NWD9 parameters (ratio doubled from NWD4)
        # =====================================================================
        self.use_nwd = getattr(h, 'use_nwd', True)
        self.nwd_ratio = getattr(h, 'nwd_ratio', 0.2)  # [NWD9] doubled from 0.1
        self.nwd_constant = getattr(h, 'nwd_constant', 3.0)  # unchanged
    ...
    def _print_config(self):
        ...
        if not hasattr(self, '_config_printed'):
            print("\n" + "=" * 60)
            print("SATAL-SWA-Plus-NWD9 Detection Loss Configuration")
            print("  (NWD4 + Higher Ratio: 0.1 → 0.2)")
            print("=" * 60)
            ...
            print(f"  [H] NWD9 Config (ratio doubled from NWD4):")
            print(f"      use_nwd:          {self.use_nwd}")
            if self.use_nwd:
                print(f"      nwd_ratio:        {self.nwd_ratio} (NWD4 was 0.1)")
                print(f"      nwd_constant:     {self.nwd_constant} (unchanged)")
