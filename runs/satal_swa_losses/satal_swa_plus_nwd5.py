...
# =============================================================================
# SATAL-SWA-Plus-NWD5: Combined Improvements over NWD4
# =============================================================================
#
# Base: NWD4
# Changes:
#   A. NWD for ALL objects (nwd_small_only=False) — fixes medium dead zone
#   B. Size-adaptive constant: C_small=2.5, C_medium=2.0, C_large=1.5
#      Tighter kernel for larger objects (better mAP50-95)
#   C. Re-enable mild small_obj_boost=1.2 (safe with ratio=0.1)
#
# =============================================================================
...
class BboxLoss(nn.Module):
    """
    Bounding box loss with SWA + NWD5 (Combined Improvements).

    NWD5 improvements over NWD4:
      - NWD for ALL objects (not just small) — fixes medium dead zone
      - Size-adaptive constant: C_small=2.5, C_medium=2.0, C_large=1.5
      - Mild small_obj_boost=1.2 re-enabled (safe with ratio=0.1)
    """
    ...
    def __init__(self, reg_max=16):
        ...
        self.use_nwd = True
        self.nwd_ratio = 0.1
        self.nwd_constant = 2.5              # [Change B] was 3.0 — tighter for small
        self.nwd_constant_medium = 2.0       # [Change B] NEW — tighter for medium
        self.nwd_constant_large = 1.5        # [Change B] NEW — tightest for large
        self.nwd_small_only = False           # [Change A] was True — all objects get NWD
        self.nwd_gate_temperature = 0.3
        self.nwd_dim_clamp = 0.5

    def normalized_wasserstein_distance_v4(self, pred, target, constant=3.0, eps=1e-7):
        """
        NWD5: Symmetric, scale-normalized Wasserstein similarity with smooth gradients.
        ...
        Args:
            ...
            constant: kernel bandwidth (larger → softer similarity), scalar or tensor
    ...
    def set_params(self, hyp):
        ...
        # [Change C] Mild boost even with NWD (safe at ratio=0.1)
        if self.use_nwd:
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes,
        ...
        """Compute IoU and DFL losses with SWA weighting + NWD5."""
        ...
        # ── NWD5 blending (all objects, size-adaptive constant) ──
        if self.use_nwd:
            ...
            if self._nwd_dbg_count < 30:
                ...
                print(f"[NWD5] nwd mean={nwd.mean().item():.4f} "
                      f"S/M/L={n_small}/{n_medium}/{n_large} "
                      f"ratio={avg_ratio:.4f} "
                      f"boost={self.small_obj_boost:.1f}")
...
class v8DetectionLoss:
    """
    SATAL-SWA-Plus-NWD5 Detection Loss.

    Base: SATAL + SWA + Class Weighting + BCE
    NWD5: Combined Improvements — all objects, size-adaptive constant, mild boost

    Key NWD5 improvements:
      - NWD for ALL objects (not just small) — fixes medium dead zone
      - Size-adaptive constant: C_small=2.5, C_medium=2.0, C_large=1.5
      - Mild small_obj_boost=1.2 re-enabled (safe with ratio=0.1)
    """
    ...
    def _print_config(self):
        ...
        if not hasattr(self, '_config_printed'):
            print("\n" + "=" * 60)
            print("SATAL-SWA-Plus-NWD5 Detection Loss Configuration")
            print("=" * 60)
            ...
            if self.use_nwd:
                ...
                print(f"      kernel:           exp(-w²/C²) [Gaussian, size-adaptive C]")
