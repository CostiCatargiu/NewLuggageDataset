...
# =============================================================================
# SATAL-SWA-Plus-NWD5a: Tighter Kernel Constant
# =============================================================================
#
# Base: NWD4
# Change: nwd_constant 3.0 → 2.5 for tighter localization (better mAP50-95)
#
# =============================================================================
...
# =============================================================================
# BBOX LOSS WITH NWD5a
# =============================================================================


class BboxLoss(nn.Module):
    """
    Bounding box loss with SWA + NWD5a (improved Normalized Wasserstein Distance).

    NWD5a improvements over NWD4:
      - Tighter kernel constant (2.5 vs 3.0) for better mAP50-95
    """
    ...
    def __init__(self, reg_max=16):
        ...
        self.nwd_constant = 2.5             # [NWD5a] was 3.0 — tighter kernel for better mAP50-95
    ...
    def normalized_wasserstein_distance_v4(self, pred, target, constant=2.5, eps=1e-7):
        """
        NWD5a: Symmetric, scale-normalized Wasserstein similarity with smooth gradients.
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes,
        ...
        """Compute IoU and DFL losses with SWA weighting + NWD5a."""
        ...
        if self.use_nwd:
            ...
            if self._nwd_dbg_count < 30:
                ...
                print(f"[NWD5a] nwd mean={nwd.mean().item():.4f} "
...
class v8DetectionLoss:
    """
    SATAL-SWA-Plus-NWD5a Detection Loss.

    Base: SATAL + SWA + Class Weighting + BCE
    NWD5a: Tighter kernel constant for better mAP50-95

    Key NWD5a improvements:
      ...
      - Tighter kernel constant (2.5 vs 3.0) for better localization
    """
    ...
    def __init__(self, model, tal_topk=10):
        ...
        # =====================================================================
        # Section H: NWD5a parameters (read from hyp, with tighter constant)
        # =====================================================================
    ...
    def _print_config(self):
        ...
        if not hasattr(self, '_config_printed'):
            print("\n" + "=" * 60)
            print("SATAL-SWA-Plus-NWD5a Detection Loss Configuration")
            print("=" * 60)
