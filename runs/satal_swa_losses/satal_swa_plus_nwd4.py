...
# =============================================================================
# SATAL-SWA-Plus-NWD4: Improved NWD Implementation
# =============================================================================
#
# Base: satal_swa_plus_nwd3.py
#
# Fixes applied over NWD3:
#   1. Symmetric normalization (geometric mean of pred/target dims)
#   2. Gaussian kernel exp(-w2/C²) instead of exp(-sqrt(w2)/C)
#      → eliminates 1/sqrt gradient singularity at perfect matches
#   3. Soft sigmoid gate instead of hard binary small_mask
#      → smooth gradient transition at size boundary
#   4. Reduced nwd_ratio default: 0.5 → 0.1
#      → NWD supplements CIoU, doesn't fight it
#   5. Increased nwd_constant: 1.0 → 3.0
#      → wider useful gradient range
#   6. Auto-disable small_obj_boost when NWD is on
#      → removes redundant quadruple small-object emphasis
#   7. Minimum dimension clamp (0.5 grid units)
#      → prevents numerical blow-up on tiny boxes
#
# =============================================================================
...
class v8DetectionLoss:
    """
    SATAL-SWA-Plus-NWD4 Detection Loss.

    Base: SATAL + SWA + Class Weighting + BCE
    NWD4: Improved Normalized Wasserstein Distance for small object regression

    Key NWD4 improvements:
    ...
    def __init__(self, model, tal_topk=10):
        ...
        # =====================================================================
        # Section H: NWD4 parameters (read from hyp, with improved defaults)
        # =====================================================================
    ...
    def _print_config(self):
        ...
        if not hasattr(self, '_config_printed'):
            print("\n" + "=" * 60)
            print("SATAL-SWA-Plus-NWD4 Detection Loss Configuration")
            print("=" * 60)
