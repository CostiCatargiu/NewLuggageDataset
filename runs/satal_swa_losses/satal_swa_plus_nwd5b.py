...
# =============================================================================
# SATAL-SWA-Plus-NWD5b: Re-enable Mild Small Object Boost
# =============================================================================
#
# Base: NWD4
# Change: small_obj_boost 1.0 → 1.2 (mild boost even with NWD active)
# Rationale: NWD4 ratio=0.1 is conservative enough that 1.2x boost 
#            won't cause the quadruple-emphasis problem NWD3 had at ratio=0.5
#
# =============================================================================
...
# =============================================================================
# BBOX LOSS WITH NWD5b
# =============================================================================


class BboxLoss(nn.Module):
    """
    Bounding box loss with SWA + NWD5b (improved Normalized Wasserstein Distance).

    NWD5b improvements over NWD4:
      - Re-enables mild small_obj_boost (1.2) even when NWD is active
      - NWD4's conservative ratio (0.1) means 1.2x boost won't cause
        the quadruple-emphasis problem that NWD3 had at ratio=0.5
    """
    ...
    def __init__(self, reg_max=16):
        ...
        # NWD5b defaults
        self.use_nwd = True
        self.nwd_ratio = 0.1               # [FIX 4] was 0.5 — now supplements CIoU
        self.nwd_constant = 3.0             # [FIX 5] was 1.0 — wider gradient range
        self.nwd_small_only = True          # gate NWD to small boxes
        self.nwd_gate_temperature = 0.3     # [FIX 3] soft gate width (fraction of threshold)
        self.nwd_dim_clamp = 0.5            # [FIX 7] minimum dimension in grid units
    ...
    def set_params(self, hyp):
        ...
        # NWD5b: Mild boost — NWD ratio is only 0.1, so 1.2x doesn't cause
        # the quadruple-emphasis that was a problem with NWD3's 0.5 ratio
        if self.use_nwd:
            # Mild boost: NWD5b ratio is only 0.1, so 1.2x doesn't cause
            # the quadruple-emphasis that was a problem with NWD3's 0.5 ratio
            self.small_obj_boost = getattr(hyp, 'small_obj_boost_nwd', 1.2)
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes,
        ...
        """Compute IoU and DFL losses with SWA weighting + NWD5b."""
        ...
        if self.use_nwd:
            ...
            if self.nwd_small_only and small_threshold is not None:
                # [FIX 3] Soft sigmoid gate — smooth transition
            ...
            if self._nwd_dbg_count < 30:
                ...
                print(f"[NWD5b] nwd mean={nwd.mean().item():.4f} "
...
class v8DetectionLoss:
    """
    SATAL-SWA-Plus-NWD5b Detection Loss.

    Base: SATAL + SWA + Class Weighting + BCE
    NWD5b: Improved Normalized Wasserstein Distance for small object regression

    Key NWD5b improvements:
      - Symmetric normalization (geometric mean)
      - Gaussian kernel (no gradient singularity)
      - Soft sigmoid gate (smooth transition)
      - Conservative ratio (supplements CIoU, doesn't replace it)
      - Mild small_obj_boost (1.2) re-enabled (safe at ratio=0.1)
    """
    ...
    def __init__(self, model, tal_topk=10):
        ...
        # =====================================================================
        # Section H: NWD5b parameters (read from hyp, with improved defaults)
        # =====================================================================
    ...
    def _print_config(self):
        ...
        if not hasattr(self, '_config_printed'):
            print("\n" + "=" * 60)
            print("SATAL-SWA-Plus-NWD5b Detection Loss Configuration")
            print("=" * 60)
            ...
            if self.use_nwd:
                ...
                print(f"      small_obj_boost:  {self.bbox_loss.small_obj_boost} (mild boost)")
