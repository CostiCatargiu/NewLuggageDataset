...
class BboxLoss(nn.Module):
    ...
    def _compute_weights(self, target_bboxes, target_scores, fg_mask, stride=None):
        ...
        fg_areas = target_areas[fg_mask]  # <-- add this
        small_mask = None  # <-- add this
        ...
        if stride is not None and area_weight.numel() > 0:
            ...
            small_mask = fg_areas < small_threshold  # reuse, no recompute

            if small_mask.any():
                area_weight = area_weight.clone()
                area_weight[small_mask] *= self.small_obj_boost

        return score_weight, area_weight, fg_areas, small_mask  # <-- 4 returns now
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes,
        ...
        score_weight, area_weight, fg_areas, small_mask = self._compute_weights(
        ...
        if self.use_nwd:
            ...
            if self.nwd_small_only and small_mask is not None:
                # per-sample ratio: nwd_ratio on small boxes, 0 elsewhere
                gate = small_mask.float().unsqueeze(-1)  # (N,1)
        ...
        if self.use_nwd:
            ...
            if self._nwd_dbg_count < 50:
                if small_mask is not None and small_mask.any():
                    sm_nwd = nwd[small_mask]
                    print(f"[NWD debug] SMALL nwd mean={sm_nwd.mean().item():.4f} "
                          f"min={sm_nwd.min().item():.4f} max={sm_nwd.max().item():.4f} "
                          f"n_small={small_mask.sum().item()}/{fg_areas.numel()}")
                else:
                    print(f"[NWD debug] no small boxes this batch "
                          f"(n_fg={fg_areas.numel()})")
