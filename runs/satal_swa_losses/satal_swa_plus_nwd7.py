...
# =============================================================================
# SATAL-SWA-Plus-NWD7: NWD for Small + Medium Objects Only
# =============================================================================
#
# Base: NWD4
#
# Key insight from experiments:
#   - NWD4 (small only): best large (0.6909), weak medium (0.7883)
#   - NWD6 (all objects): best medium (0.8099), crashed large (0.6010)
#   - NWD helps small/medium but HURTS large objects
#
# NWD7 solution:
#   - Apply NWD to small AND medium objects (soft gate up to medium threshold)
#   - Exclude large objects entirely (they revert to pure CIoU)
#   - Uses dual-threshold soft gate: full NWD < small, partial < medium, zero >= medium
#
# Expected outcome:
#   - Medium mAP50: ~0.81 (like NWD6)
#   - Large mAP50: ~0.69 (like NWD4)
#   - Best of both worlds
#
# =============================================================================
...
class BboxLoss(nn.Module):
    """
    Bounding box loss with SWA + NWD7 (NWD for small+medium only).

    NWD7 key change:
      - Applies NWD to small AND medium objects (soft gate at medium_threshold)
      - Large objects get pure CIoU (NWD hurts large object detection)
      - Combines NWD4's large object protection with NWD6's medium object improvement
    """
    ...
    def __init__(self, reg_max=16):
        ...
        self.nwd_small_medium_only = True   # NWD7: gate to small+medium, exclude large
    ...
    def normalized_wasserstein_distance_v4(self, pred, target, constant=3.0, eps=1e-7):
        ...
        Args:
            ...
            constant: kernel bandwidth (larger → softer similarity)
        ...
        # [FIX 2] Gaussian kernel: exp(-w²/C²) — no sqrt, no singularity
        #
        # Gradient: d/dw2[exp(-w2/C²)] = -exp(-w2/C²)/C²
        #   → bounded everywhere, maximum magnitude = 1/C²
        #   → no blow-up at perfect matches (w2=0)
        #
        # Compare NWD3: d/dw2[exp(-√w2/C)] = -exp(-√w2/C)/(2C√w2)
        #   → explodes as w2→0 (1/√x singularity)
    ...
    def set_params(self, hyp):
        ...
        self.nwd_small_medium_only = getattr(hyp, 'nwd_small_medium_only', self.nwd_small_medium_only)
    ...
    def _compute_soft_gate_small_medium(self, fg_areas, small_threshold):
        """
        NWD7: Soft sigmoid gate for small+medium objects only.

        Uses the MEDIUM threshold as the gate boundary (not small threshold).
        This applies NWD to both small and medium objects, but excludes large.

        Objects < small_threshold   → gate ≈ 1.0 (full NWD)
        Objects at medium_threshold → gate = 0.5 (half NWD)
        Objects > medium_threshold  → gate ≈ 0.0 (pure CIoU, large objects protected)

        The medium_threshold = small_threshold * nwd_medium_factor (default 4.0)
        """
        medium_threshold = small_threshold * self.nwd_medium_factor
        temperature = medium_threshold * self.nwd_gate_temperature
        # Clamp temperature to avoid division by zero
        temperature = max(temperature, 1e-6)
        gate = torch.sigmoid(-(fg_areas - medium_threshold) / temperature)
        return gate.unsqueeze(-1), medium_threshold  # (N, 1), scalar

    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes,
                target_scores, target_scores_sum, fg_mask, stride=None):
        """Compute IoU and DFL losses with SWA weighting + NWD7 (small+medium only)."""
        ...
        # ── NWD7 blending (small + medium objects, NOT large) ──
        if self.use_nwd:
            ...
            # Compute per-sample blending ratio
            if self.nwd_small_medium_only and small_threshold is not None:
                # NWD7: gate at MEDIUM threshold (includes small+medium, excludes large)
                gate, medium_threshold = self._compute_soft_gate_small_medium(fg_areas, small_threshold)
            ...
            if self._nwd_dbg_count < 30:
                if isinstance(ratio, float):
                    ...
                    n_small = n_medium = n_large = 0
                else:
                    avg_ratio = ratio.mean().item()
                    n_gated = (ratio > 0.01).sum().item()
                    if medium_threshold is not None:
                        n_small = (fg_areas < small_threshold).sum().item()
                        n_medium = ((fg_areas >= small_threshold) & (fg_areas < medium_threshold)).sum().item()
                        n_large = (fg_areas >= medium_threshold).sum().item()
                    else:
                        n_small = n_medium = n_large = 0
                print(f"[NWD7] nwd mean={nwd.mean().item():.4f} "
...
class v8DetectionLoss:
    """
    SATAL-SWA-Plus-NWD7 Detection Loss.

    Base: SATAL + SWA + Class Weighting + BCE
    NWD7: NWD for small + medium objects only, large objects excluded

    Key insight from NWD4/NWD6 experiments:
      - NWD helps small objects (consistent)
      - NWD helps medium objects (+1.2% mAP50 in NWD6)
      - NWD HURTS large objects (-9% mAP50 when applied to them)

    NWD7 solution:
      - Soft gate at medium_threshold (= small_threshold * 4.0)
      - Small + medium get NWD benefit
      - Large objects revert to pure CIoU (protected)
    """
    ...
    def __init__(self, model, tal_topk=10):
        ...
        # =====================================================================
        # Section H: NWD7 parameters (small+medium only, exclude large)
        # =====================================================================
        self.use_nwd = getattr(h, 'use_nwd', True)
        self.nwd_ratio = getattr(h, 'nwd_ratio', 0.1)
        self.nwd_constant = getattr(h, 'nwd_constant', 3.0)
        self.nwd_small_medium_only = getattr(h, 'nwd_small_medium_only', True)
    ...
    def _print_config(self):
        ...
        if not hasattr(self, '_config_printed'):
            print("\n" + "=" * 60)
            print("SATAL-SWA-Plus-NWD7 Detection Loss Configuration")
            print("=" * 60)
            ...
            print(f"  [H] NWD7 Config (small+medium only, large excluded):")
            print(f"      use_nwd:            {self.use_nwd}")
            if self.use_nwd:
                print(f"      nwd_ratio:          {self.nwd_ratio}")
                print(f"      nwd_constant:       {self.nwd_constant}")
                print(f"      nwd_small_med_only: {self.nwd_small_medium_only}")
                ...
                print(f"      kernel:             exp(-w²/C²) [Gaussian]")
                print(f"      gate:               soft sigmoid at medium_threshold")
